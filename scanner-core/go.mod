...
// modernc.org/sqlite was previously pinned to v1.40.1 via a replace directive to avoid a
// duplicate "sqlite" driver registration alongside grype's glebarez/sqlite. Verified
// resolved on current versions (v1.53.0), so the pin was removed and all modules track
// the same modernc.org/sqlite version.
