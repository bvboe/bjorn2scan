...
- [ ] **Agent listener: restrict the open API (bind localhost)** — the listener still binds `0.0.0.0:9999` unauthenticated (`bjorn2scan-agent/main.go`), so the read endpoints (`/api/images`, `/info`, dashboard) expose the host's full vulnerability inventory to the network, and the README still presents it as "localhost". **Mutating endpoints DONE:** `POST /api/update/{trigger,pause,resume}` are now gated behind debug mode (off by default → `403`), mirroring the scan-server's destructive-endpoint gating — `registerUpdaterHandlers` takes `debugConfig`, `test-agent-updater` runs with `debug_enabled=true`, and `agent.conf.example` documents it. **Remaining:** decide the listener bind. A blunt `127.0.0.1` default would break the remote fleet health-checks (the `validate-deployments` skill curls `:9999` across the LAN), so the realistic options are a *configurable* bind (default loopback, opt-in `0.0.0.0`) or accept-and-document the read-only exposure.
