...
- [ ] **Stage N+1 of the update-controller RBAC migration — delete the TRANSITION block** _(do this in the release **after** the one that first ships the dedicated `*-update-controller` SA; safe only once every target cluster has auto-updated through stage N)._ In `templates/clusterrole.yaml`, remove the `{{- if .Values.updateController.enabled }}` broad-perms block (configmaps/secrets/workloads/services-write/serviceaccounts/pvcs/cronjobs/jobs/clusterroles/clusterrolebindings), leaving the scan-server ClusterRole read-only (pods + pods/status + nodes + services:[get]). **Leave `templates/update-controller-rbac.yaml` untouched** — it must keep both the broad write perms *and* the mirrored scan-server read perms (pods/pods-status/nodes); removing the read mirror would make the de-privileging upgrade fail the API server's privilege-escalation check. Verified on kind that this upgrade succeeds when performed by the dedicated `*-update-controller` SA and yields the read-only scan-server SA end state. Re-run `./scripts/test-helm-upgrade-rbac` (old→N→N+1) before release.
