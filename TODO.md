...
- [x] **DONE (`cac37db`): OTLP wire compression** — gzip on both HTTP (`Content-Encoding: gzip`) and gRPC (`UseCompressor`), configurable via `otel_metrics_compression` / `OTEL_METRICS_COMPRESSION` / `otelMetrics.compression`, default `gzip`, `none` for debugging. Measured 50–66x. Verified against the live Prometheus OTLP receiver (HTTP 200, series landed).
- **Stop duplicating invariant attributes — BLOCKED, not free after all.** `deployment.name`/`deployment.uuid` sit on the OTLP Resource *and* repeat on every datapoint (48.73 MB / 13.3%). Dropping the datapoint copies is the same mistake as the rejected Resource option: `/metrics` has no Resource to fall back on, so the two paths would disagree. Listed as a trap, not a task.

- [x] **Phase 1 DONE (2026-08-19): dashboard no longer depends on any emitted composite.** All 9 expressions in `docs/grafana-container-dashboard.json` now derive `deployment_uuid_namespace_pod_container` via `label_join(...)` written to the same label name — verified identical on direct Prometheus, through Grafana's `/api/ds/query`, and re-read from the file. Also deployed live (uid `adtgfct`, version 2; v1 retained for rollback). Because same-name `label_join` is a no-op while the label is still emitted, the dashboard and code changes are **decoupled — no lockstep needed**.
- [x] **Phase 2 DONE (`0b4154a`): deleted all 6 composites from the Go code** — `hierarchicalLabels` + `buildHierarchicalLabels()` and their entries in `buildContainerBaseLabels` / `buildContainerVulnerabilityLabels`. Net deletion. Measured **22.28 MB / 6.10%** of a kubeadm scrape. Audited by exact token *and* by kind of reference: only `..._pod_container` was ever functional (9 PromQL + the `joinByField` key), and Phase 1 removed that dependency; the other five had cosmetic-only refs (`organize` column hide/order, harmless when absent). Also drop the dead `deployment_uuid_namespace_image_id` refs from the dashboard JSON, and update the 6 mentions in `docs/PROMETHEUS_METRICS.md` (the public metric contract) plus release notes.
- **`vulnerability_id` stays — deliberate (decision 2026-08-19).** It costs 41.2 MB (11.3%) and no dashboard reads it, but it exists to correlate a specific finding *at a specific point in time* across the `_vulnerability`/`_risk`/`_exploited` families; the per-rescan change pins it to a scan generation. **Do not propose removing it as dead weight.** See the doc for the churn measurements and the content-hash alternative if a stable identity is ever also wanted.
- [x] **DONE (`0b4154a`): removed the dead `deployment_uuid_namespace_image_id` reference** from the dashboard JSON, along with 10 now-dead `organize` entries for the removed labels.
- **One OTLP Resource per scanned entity — REJECTED (2026-08-19), do not re-propose.** Would have hoisted 99.96 MB / 27.4% of repeated per-datapoint identity into ~6 Resource blocks, so the headline number is tempting. Rejected because (1) it is backend-specific by construction — verified against Prometheus 3.12.0, resource attributes were dropped from the series and synthesized into `target_info` instead, and every other backend differs; (2) `/metrics` text exposition has no Resource concept, so the push and scrape paths would report different label sets; (3) the deployment-scoped half can't be dropped without breaking that consistency, and the node-scoped half is exactly what a per-deployment Resource can't carry. Full reasoning and measurements in `docs/OTEL-DATA-ARCHITECTURE.md`.
- **New design constraint recorded: the OTLP push and `/metrics` must agree.** They are two encodings of one model — a consumer must not need different queries depending on which path data arrived by. This rules out moving information between structural layers, and is why cadence and content addressing remain viable while resource-hoisting does not.

**Breaking — needs a dashboard migration (see the doc's Migration section):**
- [x] **DONE (`8900d2d` + `0b4154a`): all six composites removed** — 22.28 MB / 6.10%. (This entry previously said five were `joinByField` keys needing a multi-panel edit; that was the prefix-collision error — only `..._pod_container` was ever functional, and `label_join` handled it in a single panel. It also pointed at the per-entity Resource change as "the bigger win", which has since been rejected.)
- [x] **DONE (`f12dfcd`): per-node invariants kept only on `bjorn2scan_node_scanned`** — dropped `kernel_version`, `architecture`, `instance_type` from the node vulnerability families (46.32 MB / 12.7%), plus ~35 MB less cache memory from trimming the struct and both queries. No dashboard changes needed; `os_release` deliberately stays (the "Vulnerabilities by OS" panel groups by it directly). Guarded by `TestNodeMetricLabelPlacement`.
