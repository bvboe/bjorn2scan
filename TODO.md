...
- [x] **Default auto-update OFF for production** — WON'T DO (keep ON by decision): auto-update stays enabled by default to minimize operational overhead ("it's just an agent"). Instead, documented the trade-off for operators who want full control in `docs/AUTO_UPDATE.md` → new **RBAC & Privilege Surface** section: spells out the broad ClusterRole that `updateController.enabled` grants (cluster-wide write on `secrets` + `clusterroles`/`clusterrolebindings` = privilege-escalation surface, on the long-running scan-server SA), and how to drop it (`updateController.enabled=false` + self-driven `helm upgrade`) or constrain/audit it. **RBAC now scoped down (DONE):** the broad write perms (secrets, clusterroles/clusterrolebindings, workloads) moved off the shared scan-server SA onto a dedicated `*-update-controller` ServiceAccount + ClusterRole used only by the CronJob (`templates/update-controller-rbac.yaml`); the scan-server ClusterRole is now read-only (pods/nodes/services). So the long-running scan-server + pod-scanner no longer carry the privilege-escalation surface.
